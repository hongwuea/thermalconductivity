...
极限低温模式 = 3  # ヘリ3を用いた昇温モードの最低温度で、ヒーター電流を小さくすることでもっと低温領域の結果をとる

文件名 = f'まんがにん線2_v433_{模式}_H{磁場}T_ΔT{期望温差}K_開始{time.strftime("%H時%M分%S秒%Y年%m月%d日", time.localtime())}'
线程锁1 = Lock()
初始时间 = time.time()
数据表 = [热浴时间表, 热浴温度表, 样品时间表, 样品温差表, 结果温度表, 结果热导表, 拟合时间表, 拟合温差表, 切分表] \
    = [[] for _ in range(9)]
...
def G118κsweep热电偶测量():
    ...
    while 1:
        ...
        真空度 = np.mean([K2182真空計.读电压() for _ in range(5)])
        print(f'κ={κ},晶中温度={晶中温度}K,延迟时间={延迟时间}s,时间常数={τ}s,{τ降}s,设定电流={设定电流}A,真空度={真空度}V')
        with 线程锁1:
            结果温度表.append(晶中温度)
            结果热导表.append(κ)
        结果文件.write(f'{κ}\t{晶中温度}\t{延迟时间}\t{τ}\t{设定电流}\t{低温侧相对热浴温升}\t{真空度}\n')
        结果文件.flush()
...
def G118κstep热电偶测量():  # 待验证
    ...
    while (设定温度 - 終了温度) * (设定温度 - 初始温度) <= 0:
        ...
        for i in range(平均回数):
            ...
            升温整理表 = [np.array(升温时间表) - 时间_设电流高,
                     (np.array(升温温差表) - 线性背景拟合(升温时间表)) / ΔT, ]
            降温整理表 = [np.array(降温时间表) - 时间_设电流低,
                     (np.array(降温温差表) - 线性背景拟合(降温时间表)) / ΔT, ]
            try:
                [延迟时间, τ] = curve_fit(升温拟合, *升温整理表, p0=[1, τ], bounds=([-10, 0.05], [10, 50]))[0]
                [延迟时间降, τ降] = curve_fit(降温拟合, *降温整理表, p0=[1, τ], bounds=([-10, 0.05], [10, 50]))[0]
            except:
...
def G118κstepRuO2测量():  # 待验证
    设定温差, 设定温度, 设定电流, τ, 极限低温重复 = 期望温差, 初始温度, 初始电流, τ_0, 极限低温模式
    while (设定温度 - 終了温度) * (设定温度 - 初始温度) <= 0:
        K6220加热器.设电流t(0)
        if 极限低温重复 <= 0:
            热浴稳定(设定温度, 热浴, Ls350_1, 3, 4e-4, 2e-4)
        else:
            极限低温重复 -= 1
            设定温差 = 期望温差 * (极限低温模式 - 极限低温重复) / 极限低温模式
        读350(7 * max(2, τ) * 3)
        ...
        for i in range(平均回数):
            ...
            降温时间表, 降温高温表 = 降温表[0]
            # 降温低温表 = 降温表[1][1]
            高温rel2, 时间rel2, 低温rel2 = np.mean(高温表[-安定点数:]), np.mean(高温时间表[-安定点数:]), np.mean(
                低温表[-安定点数:])
            ...
            升温整理表 = [np.array(升温时间表) - 时间_设电流高,
                     (np.array(升温高温表) - 线性背景拟合(升温时间表)) / ΔT高, ]
            降温整理表 = [np.array(降温时间表) - 时间_设电流低,
                     (np.array(降温高温表) - 线性背景拟合(降温时间表)) / ΔT高, ]
            try:
                [延迟时间, τ] = curve_fit(升温拟合, *升温整理表, p0=[1, τ], bounds=([-10, 0.05], [10, 50]))[0]
                [延迟时间降, τ降] = curve_fit(降温拟合, *降温整理表, p0=[1, τ], bounds=([-10, 0.05], [10, 50]))[0]
            ...
            with 线程锁1:
                ...
                高温拟合温差表.extend(
                    ΔT高 * 升温拟合(np.array(升温时间表) - 时间_设电流高, 延迟时间, τ) + 线性背景拟合(升温时间表))
                高温拟合温差表.extend(
                    ΔT高 * 降温拟合(np.array(降温时间表) - 时间_设电流低, 延迟时间降, τ降) + 线性背景拟合(降温时间表))
            切分表.append(len(样品时间表))
            ...
            with 线程锁1:
                ...
                结果热导表.append(κ)
            结果文件.write(
                f'{κ}\t{晶中温度}\t{延迟时间}\t{τ}\t{设定电流}\t{ΔT高}\t{ΔT低}\t{加热器高电压}\t{加热器零电压}\n')
            结果文件.flush()
            校正文件.write(f'{np.mean(热浴温度表[-5:])}\t{高温rel1}\t{低温rel1}\n')
            校正文件.flush()
            设定电流 = min(设定电流 * ((设定温差 / abs(ΔT) * min(1, np.mean(热浴温度表[-5:]) / 40)) ** 0.5), 3e-4)
        if 极限低温重复 <= 0:
            设定温度 = 设定温度 - 降温間隔 * min(1, 设定温度 / 40)
...
if 模式 == 'G118κsweep热电偶测量' or 模式 == 'G118κstep热电偶测量':
    ...
    结果文件.write('{κ}\t{晶中温度}\t{延迟时间}\t{τ}\t{设定电流}\t{低温侧相对热浴温升}\t{真空度}\n')
...
if 模式 == 'G118κstepRuO2测量':
    ...
    if 1:  # 窗口内图3级
        ...
        if 1:  # 窗口内曲线4级
            ...
            あてはめ曲線 = 右图.plot(高温拟合时间表, 高温拟合温差表, pen='g', name='あてはめ', symbol='o',
                             symbolBrush='g')
