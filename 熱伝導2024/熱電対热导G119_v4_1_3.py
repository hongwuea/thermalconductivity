...
热浴 = ['B', 'X86763', 'X86763逆', 2, 1]  # Ls350格式，输入号，校正数据，逆校正，输出加热序号，磁场校正
RuO2低 = ['C', '热导左1T', '', 0, 1]
RuO2高 = ['D', '热导右1T', '', 0, 1]
...
文件名 = f'{模式}まんがにん線2ΔT{期望温差}K{time.strftime("%H時%M分%S秒%Y年%m月%d日", time.localtime())}開始'
线程锁1 = Lock()
初始时间 = time.time()
...
def 读350(回数):
    ...
    for _ in range(int(回数)):
        ...
        with (线程锁1):
            ...
            低温表.append(小低温表[-1])
    return [小高温表, 小低温表, 小高温時間表, 小低温時間表]
...
def G118κsweep热电偶测量():
    ...
    while 1:
        ...
        读纳伏表点数, 安定点数 = int(6 * max(1, τ) * 3), int(6 * max(1, τ) * 3)
        ...
        try:
            [延迟时间, τ] = curve_fit(升温拟合, *升温整理表, p0=[1, τ])[0]
            [延迟时间降, τ降] = curve_fit(降温拟合, *降温整理表, p0=[1, τ])[0]
        except:
            print('フィッチング失敗！')
            延迟时间, τ = 0, 10
            延迟时间降, τ降 = 0, 10
        ...
        with 线程锁1:
            结果温度表.append(晶中温度)
            结果热导表.append(κ)
        结果文件.write(f'{κ}\t{晶中温度}\t{延迟时间 - 时间_设电流高}\t{τ}\t{设定电流}\t{低温侧相对热浴温升}\n')
        结果文件.flush()
...
def G118κstepRuO2测量():  # 待完成，只拟合高温
    ...
    while (设定温度 - 終了温度) * (设定温度 - 初始温度) <= 0:
        ...
        for i in range(平均回数):
            ...
            加热器零电压 = K2182加热器.读电压()
            时间_设电流高 = K6220加热器.设电流t(设定电流) - 初始时间
            读350点数, 安定点数 = int(6 * max(1, τ) * 3), int(2 * max(1, τ) * 3)
            升温时间表, 升温温差表 = 读350(读350点数)
            低温侧相对热浴温升 = abs(K2182热电偶.读通道2后切回() - REL低温修正) / 塞贝克系数(热浴温度表[-1])
            加热器高电压 = K2182加热器.读电压()
            时间_设电流低 = K6220加热器.设电流t(0) - 初始时间
            降温时间表, 降温温差表 = 读350(读350点数)
            温差rel2, 时间rel2 = np.mean(样品温差表[-安定点数:]), np.mean(样品时间表[-安定点数:])
            ΔT = np.mean(升温温差表[-安定点数:]) - (温差rel1 + 温差rel2) / 2
...
if 模式 == 'G118κsweep热电偶测量' or 'G118κstep热电偶测量':
    # pg全局1级
    ...
    if 1:  # 窗口内图3级
        ...
        右图.addLegend()
        if 1:  # 窗口内曲线4级
            高侧曲线 = 右图.plot(样品时间表, 样品温差表, pen='b', name='高', symbol='o', symbolBrush='b')
    ...
    结果文件.write('κ\t晶中温度\t延迟时间\t时间常数\t设定电流\t低温侧相对热浴温升\n')
    Thread(target=G118κsweep热电偶测量, daemon=True).start()
...
if 模式 == 'G118κstepRuO2测量':
    ...
    过程文件.write(str([热浴时间表, 热浴温度表, 高温时间表, 高温表, 低温时间表, 低温表, 结果温度表, 结果热导表, 切分表]) + '\n')
    过程文件.close()
    结果文件.close()
    print('終わり')
