...
if __name__ == '__main__':
    ...
    if not os.path.exists(r'結果'):
        os.makedirs(r'結果')
    曲線ファイル = open(f'結果/断熱消磁{time.strftime("%H時%M分%S秒%Y年%m月%d日", time.localtime())}.txt', mode='a', encoding='utf-8')
    曲線ファイル.write('[時間表, 高侧表, 低侧表, 磁场表]\n')
    曲線ファイル.write(str([时间表, 高侧表, 低侧表, 磁场表]))
    曲線ファイル.write('\n')
    曲線ファイル.flush()
    曲線ファイル.close()
    print("おわり")
