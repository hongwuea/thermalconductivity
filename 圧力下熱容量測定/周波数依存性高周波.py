...
def 計測():
    for i in range(点数):
        ...
        time.sleep(10)
        for _ in range(平均次数):
            OUT = SR850_1.读取('X') + 1j * SR850_1.读取('Y')
            time.sleep(1)
            with スレッドロック1:
                频率表.append(频率)
                幅值表.append(abs(OUT))
                角度表.append(np.angle(OUT, True))
...
if __name__ == '__main__':
    ...
    if 1:  # 窗口内图3级
        ...
        右图2.setLogMode(x=True, y=True)
        if 1:  # 窗口内曲线4级
            R = 右图2.plot(频率表, 幅值表, pen='b', name='R表', symbol='o', symbolBrush='b')
    if 1:  # 窗口内图3级
        右图3 = 窗口.addPlot(title="内部pt100温度計")
        右图3.setLabel(axis='left', text='φ値/°')
        右图3.setLabel(axis='bottom', text='周波数/Hz', )
        右图3.setLogMode(x=True, y=None)
        if 1:  # 窗口内曲线4级
            φ = 右图3.plot(频率表, 角度表, pen='b', name='φ表', symbol='o', symbolBrush='b')
    ...
    for i in range(len(频率表)):
        結果.write(f'{频率表[i]}\t{幅值表[i]}\t{角度表[i]}\n')
    結果.flush()
    結果.close()
    print('完了しました！')
