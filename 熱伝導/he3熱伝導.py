...
def 测定():
    ...
    if not os.path.exists(r'结果'):
        os.makedirs(r'结果')
    平均前文件 = open(f'结果/平均前低温{time.strftime("%H時%M分%S秒%Y年%m月%d日", time.localtime())}.txt', mode='a', encoding='utf-8')
    ...
    while (设定温度 - 終了温度) * (设定温度 - 初始温度) <= 0:
        ...
        for i in range(平均回数):
            print(f'温度{设定温度}K下的第{i + 1}次测量开始,新设定点数={50}个，设定电流={设定电流:e}A\n开始取REL')
            ...
            高低温侧取值(30)
            REL = np.mean(高侧表[-30::]) - np.mean(低侧表[-30::])
            print(f'REL={REL}K，开始升温。。。')
            K6220_1.设电流(设定电流)
            高低温侧取值(50)
            晶中温度 = (np.mean(高侧表[-30:]) + np.mean(低侧表[-30:])) / 2
            ΔT = np.mean(高侧表[-30:]) - np.mean(低侧表[-30:]) - REL
            ...
            print(f'设定温度={设定温度}\t温度差={ΔT}K\t相对热导{κ}V/W\t时间常数={τ}s\t时间偏离={t_0}s\n结束小循环')
            print(f'等待10秒降温缓和')
...
if __name__ == '__main__':
    ...
    if 1:  # 窗口内图3级
        ...
        if 1:  # 窗口内曲线4级
            ...
            低温侧曲线 = 右图.plot(高低时间表, 低侧表, pen='r', name='低', symbol='o', symbolBrush='b')
