...
初始电流 = 30 * 10 ** -6  # 开始电流
期望温差 = 1  # 加熱による温度差、低温では小さくなる
初始点数 = 100

初始时间 = time.time()
数据表 = [时间表, 温度表, 高低时间表, 高侧表, 低侧表, 结果温度热电偶, 结果热导热电偶, 结果温度电阻, 结果热导电阻] = [[] for _ in range(9)]
线程锁1 = Lock()
ファイル名 = '切换'
    ...
    while (设定温度 - 終了温度) * (设定温度 - 初始温度) <= 0:
            ...
            for i in range(平均回数):
                ...
                def 高低温侧取值(次数):
                    for _ in range(次数):
                        小循环时间 = time.time()
                        低温侧 = 0
                        高温侧 = K2182_1.读电压() / 塞贝克系数(设定温度 + 期望温差/2)
                        with 线程锁1:
                            高侧表.append(高温侧)
                            低侧表.append(低温侧)
                ...
                K6220_1.设电流(设定电流)
                高低温侧取值(设定点数)  # 修正必要
                ΔT = np.mean(高侧表[-80:]) - REL
                晶中温度 = 设定温度 + (期望温差 * min(1, (设定温度 ** 1.5 / 250))) / 2 + abs(K2182_1.读通道2后切回() - 低温侧REL) / 塞贝克系数(设定温度)
                # 测量功率
        ...
        if 设定温度 < 切换温度高:  # 电阻测量
            ...
            for i in range(平均回数):
                ...
                高低温侧取值(设定点数)
                取点数 = 设定点数 - 20
                REL = np.mean(高侧表[-取点数::]) - np.mean(低侧表[-取点数::])
                # 未进行稳定判定，考虑使用一次方拟合来判断稳定性，并且增加重复次数以提高精度
                print(f'REL={REL}K，开始升温。。。')
                K6220_1.设电流(设定电流)
                高低温侧取值(设定点数)
                晶中温度 = (np.mean(高侧表[-取点数:]) + np.mean(低侧表[-取点数:])) / 2
                ΔT = np.mean(高侧表[-取点数:]) - np.mean(低侧表[-取点数:]) - REL
                # 测量功率
                加热器正电压 = np.mean([K199_1.读电压() for _ in range(5)])
                K6220_1.设电流(-1 * 设定电流)
                time.sleep(3)
        ...
        结果文件.write(f'{int(time.time() - 初始时间)}\t{设定温度}\t{np.mean(κ电阻表)}\t{np.mean(κ热电偶表)}\n')
        结果文件.flush()
...
if __name__ == '__main__':
    ...
    if 1:  # 窗口内图3级
        ...
        结果图.addLegend()
        if 1:  # 窗口内曲线4级
            结果曲线 = 结果图.plot(结果温度热电偶, 结果热导热电偶, pen='c', name='热电偶', symbol='o', symbolBrush='b')
            结果曲线2 = 结果图.plot(结果温度电阻, 结果热导电阻, pen='c', name='电阻', symbol='o', symbolBrush='r')
    ...
    def 定时更新f():
        with 线程锁1:
            ...
            结果曲线.setData(结果温度热电偶, 结果热导热电偶)
            结果曲线2.setData(结果温度电阻, 结果热导电阻)
