...
def 测定():
    ...
    while 1:
        ...
        if 温度表[-1] > 50:
            try:
                拟合参数表, 协方差矩阵 = curve_fit(lambda 时间, 电压差, 时间常数, 时间偏移: 电压差 * (1 - np.exp(-(时间 - 时间偏移) / 时间常数))
                                         , 拟合时间列, 拟合温差列, p0=拟合参数表)
            except:
                print(sys.exc_info())
        ...
        print(f'单次测量结束，时间={int(time.time() - 初始时间)}s\t'
              f'设定温度={设定温度}\t温度差={ΔT}K\t相对热导{κ}W/K\t时间常数={τ}s\t时间偏离={t_0}s\n结束小循环')
        print(f'等待{max(30, min(5 * τ, 600)):.4g}秒降温缓和')
        # 设定电流 = min(设定电流 * ((期望温差 / ΔT) ** 0.5), 2E-4)
        设定电流 = min(设定电流 * ((期望温差 / ΔT) ** 0.5), 0.0002)
