...
def 热浴稳定he3(温度, 通道):  # 用二次函数拟合最近2分钟内的取的128个点，一次和二次项小于某一限值即可，并输出过程中的温度表和稳定时间
    Ls350_1.设温度(温度计转换(温度, '热浴逆'), 通道=通道)
...
def 温度计转换(输入, 转换号):
    ...
    转换['低温侧'], 转换['高温侧'] = 转换['低温侧1T_L'], 转换['高温侧1T_L']
    号 = 转换[转换号]
    Amax, Amin, 底 = 号[0:3]
    チェビシェフ係数 = 号[3:]
    try:
...
def 温度计转换简易(输入, 转换号):
    转换 = {
        '高温侧0T': [10561.96288, 2.46684695],
        '低温侧0T': [10891.84685, 2.522445166],
    }
    转换['低温侧'], 转换['高温侧'] = 转换['低温侧0T'], 转换['高温侧0T']
    return 转换[转换号][1]/(输入/转换[转换号][0]-1)
