...
# def 新塞贝克系数(塞_温度):  # 温度→V/K
#     a3 = 0.000001918636149
#     a2 = -0.001290978061813
#     a1 = 0.426821584524458
#
#     return 1E-06 * (a3 * 塞_温度 ** 3 + a2 * 塞_温度 ** 2 + a1 * 塞_温度)
#
#
# def 新塞贝克系数2(塞_温度):  # 温度→V/K
#     a3 = 0.000000861546980
#     a2 = -0.000747632549346
#     a1 = 0.343035500802204
#     a0 = 3.718462326469690
#
#     return 1E-06 * (a3 * 塞_温度 ** 3 + a2 * 塞_温度 ** 2 + a1 * 塞_温度 + a0)


# def 新塞贝克系数3(塞_温度):  # 温度→V/K
#     a13 = -2.87497167957563E-27
#     a12 = 4.86941393030502E-24
#     a11 = -3.57787964901576E-21
#     a10 = 1.48015604610740E-18
#     a9 = -3.70760795512144E-16
#     a8 = 5.44986965633722E-14
#     a7 = -3.43814699991104E-12
#     a6 = -2.66534040237476E-10
#     a5 = 7.66411509250577E-08
#     a4 = -7.19031642658282E-06
#     a3 = 0.000355005770531619
#     a2 = -0.0105835760065069
#     a1 = 0.541734278127885
#
#     return 1E-06 * (a13 * 塞_温度 ** 13 + a12 * 塞_温度 ** 12 + a11 * 塞_温度 ** 11 + a10 * 塞_温度 ** 10 + a9 * 塞_温度 ** 9 + a8 * 塞_温度 ** 8 + a7 * 塞_温度 ** 7 + a6 * 塞_温度 ** 6 + a5 * 塞_温度 ** 5 + a4 * 塞_温度 ** 4 + a3 * 塞_温度 ** 3 + a2 * 塞_温度 ** 2 + a1 * 塞_温度 ** 1)


def 塞贝克系数4(塞_温度):  # 温度→V/K
    a12 = -3.4657842013E-26
    a11 = -5.5827328721E-23
    a10 = -3.9673619516E-20
    a9 = -1.6414776355E-17
    a8 = -4.3979497391E-15
    a7 = -8.0370123621E-13
    a6 = -1.0287605534E-10
    ...
    return 1E-06 * (a12 * (塞_温度 - 273.15) ** 12 * 13 + a11 * (塞_温度 - 273.15) ** 11 * 12 + a10 * (塞_温度 - 273.15) ** 10 * 11 + a9 * (塞_温度 - 273.15) ** 9 * 10 + a8 * (塞_温度 - 273.15) ** 8 * 9 + a7 * (塞_温度 - 273.15) ** 7 * 8 + a6 * (塞_温度 - 273.15) ** 6 * 7 + a5 * (塞_温度 - 273.15) ** 5 * 6 + a4 * (塞_温度 - 273.15) ** 4 * 5 + a3 * (塞_温度 - 273.15) ** 3 * 4 + a2 * (塞_温度 - 273.15) ** 2 * 3 + a1 * (塞_温度 - 273.15) ** 1 * 2 + a0)


def ペルチェ系数4(塞_温度):  # 温度→V/K
    return 塞_温度 * 塞贝克系数4(塞_温度)

# def 積分塞贝克系数(塞_温度):  # 温度→V/K
#     a3 = 2.034140227
#     a2 = -1.341522423
#     a1 = 0.432616895
#
#     return 1E-06 * (1E-06 * a3 / 4 * 塞_温度 ** 4 + 1E-03 * a2 / 3 * 塞_温度 ** 3 + a1 / 2 * 塞_温度 ** 2)


def 新積分塞贝克系数2(塞_温度):  # 温度→V/K
    a3 = 0.000000861546980
    a2 = -0.000747632549346
    a1 = 0.343035500802204
    a0 = 3.718462326469690

    return 1E-06 * (a3 / 4 * 塞_温度 ** 4 + a2 / 3 * 塞_温度 ** 3 + a1 / 2 * 塞_温度 ** 2 + a0 * 塞_温度)
#
#
# def 新塞贝克系数2積分(塞_温度1, 塞_温度2):
#     return 新積分塞贝克系数2(塞_温度2) - 新積分塞贝克系数2(塞_温度1)


# def 新積分塞贝克系数3(塞_温度):  # 温度→V/K
#     a13 = -2.87497167957563E-27
#     a12 = 4.86941393030502E-24
#     a11 = -3.57787964901576E-21
#     a10 = 1.48015604610740E-18
...
#     a1 = 0.541734278127885
#
#     return 1E-06 * (a13 * 塞_温度 ** 14 / 14 + a12 * 塞_温度 ** 13 / 13 + a11 * 塞_温度 ** 12 / 12 + a10 * 塞_温度 ** 11 / 11 + a9 * 塞_温度 ** 10 / 10 + a8 * 塞_温度 ** 9 / 9 + a7 * 塞_温度 ** 8 / 8 + a6 * 塞_温度 ** 7 / 7 + a5 * 塞_温度 ** 6 / 6 + a4 * 塞_温度 ** 5 / 5 + a3 * 塞_温度 ** 4 / 4 + a2 * 塞_温度 ** 3 / 3 + a1 * 塞_温度 ** 2 / 2)


# def 新塞贝克系数3積分(塞_温度1, 塞_温度2):
#     return 新積分塞贝克系数3(塞_温度2) - 新積分塞贝克系数3(塞_温度1)


def 積分塞贝克系数4(塞_温度):  # 温度→V/K
    a13 = -3.4657842013E-26
    a12 = -5.5827328721E-23
    a11 = -3.9673619516E-20
    a10 = -1.6414776355E-17
    a9 = -4.3979497391E-15
    a8 = -8.0370123621E-13
    a7 = -1.0287605534E-10
    a6 = -9.3214058667E-9
    ...
    return 1E-06 * (a13 * (塞_温度 - 273.15) ** 13 + a12 * (塞_温度 - 273.15) ** 12 + a11 * (塞_温度 - 273.15) ** 11 + a10 * (塞_温度 - 273.15) ** 10 + a9 * (塞_温度 - 273.15) ** 9 + a8 * (塞_温度 - 273.15) ** 8 + a7 * (塞_温度 - 273.15) ** 7 + a6 * (塞_温度 - 273.15) ** 6 + a5 * (塞_温度 - 273.15) ** 5 + a4 * (塞_温度 - 273.15) ** 4 + a3 * (塞_温度 - 273.15) ** 3 + a2 * (塞_温度 - 273.15) ** 2 + a1 * (塞_温度 - 273.15) ** 1)
...
def 熱電対温度4(塞_温度, 熱電圧):
    sp.var('x')
    Sol = sp.solve(積分塞贝克系数4(x) - 積分塞贝克系数4(塞_温度) - 熱電圧, x)
    return float(Sol[1])


def 熱電対温度4f(塞_温度, 熱電圧):
    ΔT1 = 熱電圧 / 塞贝克系数4(塞_温度)
    ΔT2 = 熱電圧 / 塞贝克系数4(塞_温度 + ΔT1)
    return 塞_温度 + (ΔT1 + ΔT2) / 2


# def 熱電対温度(塞_温度, 熱電圧):
#     sp.var('x')
#     Sol = sp.solve(積分塞贝克系数(x) - 積分塞贝克系数(塞_温度) - 熱電圧, x)
#     return float(Sol[1])
...
#     Sol = sp.solve(積分塞贝克系数(x) - 積分塞贝克系数(塞_温度) - 熱電圧, x)
#     # return float(Sol[1])
#     return float(Sol[1]) - 塞_温度


def 新熱電対温度差2(塞_温度, 熱電圧):
    sp.var('x')
    Sol = sp.solve(新積分塞贝克系数2(x) - 新積分塞贝克系数2(塞_温度) - 熱電圧, x)
    # return float(Sol[1])
    return float(Sol[1])
...
def 热浴稳定(温度, 热浴, 温控器=Ls350_1):  # 用二次函数拟合最近2分钟内的取的128个点，一次和二次项小于某一限值即可，并输出过程中的温度表和稳定时间
    通道 = 热浴[3]
...
def 热浴稳定he3(温度, 热浴, 温控器=Ls350_1):  # 用二次函数拟合最近2分钟内的取的128个点，一次和二次项小于某一限值即可，并输出过程中的温度表和稳定时间
    通道 = 热浴[3]
    温控器.设温度(温度计转换(温度, 热浴[2]), 通道=通道)
    温控器.设PID(P=(温度 / 10) * 10 ** (3 - 温控器.读加热量程(通道=通道) / 2),
             I=max(10, min(100, 16000 / 温度 ** 2)), 通道=通道)
...
def 热浴稳定Ls370(温度, 热浴, 温控器):  # 用二次函数拟合最近2分钟内的取的128个点，一次和二次项小于某一限值即可，并输出过程中的温度表和稳定时间
    温控器.设温度(温度计转换(温度, 热浴[2]))
    温控器.设PID(P=15 * 温度 ** 0.5, I=max(10, 温度), )
    print('开始检查温度稳定性')
    稳定标志 = 0
    while 稳定标志 < 稳定次数:
        功率表, 热浴稳定温度表, 电阻表 = [], [], []
...
# # Ls350を外部から導入
# def 熱浴安定he3(温度, 热浴):  # 用二次函数拟合最近2分钟内的取的128个点，一次和二次项小于某一限值即可，并输出过程中的温度表和稳定时间
#     通道 = 热浴[3]
