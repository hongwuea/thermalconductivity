...
def 热浴稳定Ls350(温度, 热浴, 温控器, 稳定次数=3, 变动上限=2e-4, 误差上限=1e-4):  # 用二次函数拟合最近2分钟内的取的128个点，一次和二次项小于某一限值即可，并输出过程中的温度表和稳定时间
    通道 = 热浴[3]
    温控器.设温度(温度计转换(温度, 热浴[2]), 通道=通道)
    温控器.设PID(P=(温度 / 10) * 10 ** (3 - 温控器.读加热量程(通道=通道) / 2),
             I=max(3, min(150, 3 * 30 ** 3 / 温度 ** 3)), 通道=通道)
    ...
    while 稳定标志 < 稳定次数:
        功率表, 热浴稳定温度表, 电阻表 = [], [], []
        for _ in range(int(max(10, min(64, 温度 * 2)))):
            电阻 = 温控器.读电阻(通道=热浴[0])
            电阻表.append(电阻)
            热浴稳定温度表.append(温度计转换(电阻, 热浴[1]))
            time.sleep(0.3)
...
def ファイル保存(ファイル名, 表記, 表):
    if not os.path.exists(f'結果'):
        os.makedirs(f'結果')
    結果 = open(f'結果/{ファイル名}{time.strftime("%Y年%m月%d日%H時%M分%S秒", time.localtime())}_.txt', mode='a',
              encoding='utf-8')
    結果.write(表記)
