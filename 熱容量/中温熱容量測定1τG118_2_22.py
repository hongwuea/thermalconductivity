...
def 計測():
    ...
    while (设定温度 - 終了温度) * (设定温度 - 初始温度) <= 0:
        ...
        for i in range(平均回数):
            ...
            ΔT, κ, T_0 = 加熱過程()  # 安定化判断
            電流 = 電流 * (期待温昇 / ΔT * min(1, (设定温度 / 40))) ** 0.5  # 電流調整
            ...
            try:
                当てはめてみる = curve_fit(
                    lambda t_l, τ_l, t0_l: ΔT * np.exp(-(lambda x: (abs(x) + x) / 2)(t_l - 時間_0 + t0_l) / τ_l) + T_0,
                    *緩和曲線, p0=[τ, 0])  #
                # フィッティング
            except:
                pass
...
def 定時更新f():
    ...
    # time.sleep(3)
    # with スレッドロック1:
    #     熱浴温度曲線2.setData(熱浴時間表[-1000::], 熱浴温度表[-1000::])
    #     試料温度曲線2.setData(時間表[-1000::], 温度差表[-1000::])
    #     当てはめ2.setData(当てはめ時間表[-1000::], 当てはめ曲線[-1000::])

    num = min(len(熱浴時間表), 1000)
    # time.sleep(3)
    with スレッドロック1:
        熱浴温度曲線2.setData(熱浴時間表[-num::], 熱浴温度表[-num::])
...
if not os.path.exists(r'結果'):  # データ保存
    os.makedirs(r'結果')
曲線ファイル = open(f'結果/高温熱容量{time.strftime("%H時%M分%S秒%Y年%m月%d日", time.localtime())}運行時間{(time.time() - 初期時間) / 3600}h.txt',
