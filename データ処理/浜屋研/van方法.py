...
if __name__ == '__main__':
    ...
    if 1:  # 窗口内图3级
        ...
        if 1:  # 窗口内曲线4级
            ...
            偶成分曲线 = 左图.plot(磁场插值采样, 电压偶成分, pen='g', name='奇成分', symbol='o', symbolBrush='g')
    ...
    # 窗口2级
    窗口2 = pg.GraphicsLayoutWidget(show=True, title="断熱消磁")
    窗口2.resize(800, 500)
    if 1:  # 窗口内图3级
        左图 = 窗口2.addPlot(title="磁場ー電圧")
        左图.setLabel(axis='left', text='電圧/V')
        左图.setLabel(axis='bottom', text='磁場/Oe', )
        if 1:  # 窗口内曲线4级
            电压曲线 = 左图.plot(磁场表, 电压表, pen='g', name='磁場対電圧', symbol='o', symbolBrush='b')
