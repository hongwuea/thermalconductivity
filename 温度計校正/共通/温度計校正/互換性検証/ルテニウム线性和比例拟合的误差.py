...
热浴温度, R1, R2, R3 = [表[::-1] for 表 in 读取txt('3個ルテニウム温度計.txt')]
# 热浴温度, R1, R2, R3 = 读取txt('3個ルテニウム温度計.txt')
print(R1, 热浴温度)
# 温度, 电阻 = 读取txt('lakeshore103.txt')
# 电阻2, 温度2 = 读取txt('中澤0.08-3.6K.txt')
标准函数 = CubicSpline(R1, 热浴温度)
温度残差 = 标准函数(R2 * R1[0] / R2[0]) - 热浴温度
温度残差2 = 标准函数(R3 * R1[0] / R3[0]) - 热浴温度
# 温度残差2 = 热浴温度 - 标准函数(R2 * R1[20] / R2[20])
# R32 = R3*R1[-1]/R3[-1]
...
轴1.plot(热浴温度, 温度残差, label='KOA10kΩ0402_R2', marker='*', markersize=2)
轴1.plot(热浴温度, 温度残差2, label='KOA10kΩ0402_R1', marker='*', markersize=2)
# [轴1.plot(温度, 电阻, label=r'lakeshore10kΩ0805', marker='s', markersize=2), # 轴2.plot(差分横轴(差分横轴(温度)), np.diff(np.diff(
# 电阻) / np.diff(温度))/np.diff(差分横轴(温度)), label=r'lakeshore10kΩ080', marker='*', #         markersize=2) ]
# for i in [(R1, 'R1'), (R2, 'R2'), (R3, 'R3')]:
#     曲线表.append(轴1.plot(热浴温度, i[0], label='KOA10kΩ0402_' + i[1], marker='*', markersize=2))
#     #     曲线表.append(轴2.plot(差分横轴(热浴温度), -np.diff(np.log(i[0])) / np.diff(np.log(热浴温度)),
#     #                        label='KOA10kΩ0402_' + i[1], marker='*', markersize=2))
#     曲线表.append(
#         轴2.plot(差分横轴(差分横轴(热浴温度)), 差分横轴(差分横轴(热浴温度)) ** 3 * np.diff(np.diff(i[0]) / np.diff(热浴温度)) // np.diff(差分横轴(热浴温度)),
#                 label='KOA10kΩ0402_' + i[1], marker='*', markersize=2))
# # 曲线表.append(轴2.plot(差分横轴(温度2), -np.diff(np.log(电阻2)) / np.diff(np.log(温度2)),
# #                    label='nakazawa', marker='*', markersize=2))

设置轴(轴1, r'T/K', r'errorT/K', 'one point calibration by R1 residual error')
设置log轴(轴2, r'T/K', r'errorT/K', ' ')
