...
曲线表 = [轴1.plot(温度, 电阻, label=r'lakeshore10kΩ0805', marker='s', markersize=2),
       # 轴2.plot(差分横轴(差分横轴(温度)), np.diff(np.diff(电阻) / np.diff(温度))/np.diff(差分横轴(温度)), label=r'lakeshore10kΩ080', marker='*',
       #         markersize=2)
       ]
...
#                        label='KOA10kΩ0402_' + i[1], marker='*', markersize=2))
    曲线表.append(轴2.plot(差分横轴(差分横轴(热浴温度)), 差分横轴(差分横轴(热浴温度))**3*np.diff(np.diff(i[0]) / np.diff(热浴温度))//np.diff(差分横轴(热浴温度)),
                       label='KOA10kΩ0402_' + i[1], marker='*', markersize=2))
