...
曲线表 = [轴1.plot(温度, 电阻, label=r'lakeshore10kΩ0805', marker='s', markersize=2),
       轴2.plot(差分横轴(温度), -np.diff(np.log(电阻)) / np.diff(np.log(温度)), label=r'lakeshore10kΩ080', marker='*',
               markersize=2)]
...
曲线表.append(轴2.plot(差分横轴(温度yu), -np.diff(np.log(电阻yu1)) / np.diff(np.log(温度yu)),
                       label='KOA1kΩ0603yu1', marker='*', markersize=2))
曲线表.append(轴2.plot(差分横轴(温度yu), -np.diff(np.log(电阻yu2)) / np.diff(np.log(温度yu)),
                       label='KOA1kΩ0603yu2', marker='*', markersize=2))
# 轴3.plot(温度yu, label='nakazawa', marker='*', markersize=2)
设置轴(轴1, r'T/K', r'R/Ω', 'T/R')
设置log轴(轴2, r'T/K', r'dlnR/dlnT', 'T/R')
