...
R表 =(X表**2+Y表**2)**0.5
Cp表 = 塞贝克系数(温度表) / R表
θ表 = math.degrees(math.atan2(Y表, X表))
时间表 = 时间表-时间表[0]
...
#        正交Y = 右图.plot(温度表, Y表, pen='b', name='正交Y', symbol='o', symbolBrush='b')
    ...
    if 1:  # 窗口内曲线4级
        ...
        R = 右图2.plot(温度表, R表, pen=None, name='R表', symbol='o', symbolBrush='b', symbolSize=10)

    右图4 = 窗口.addPlot(title="θ表")
    右图4.setLabel(axis='left', text='角度/°')
    右图4.setLabel(axis='bottom', text='温度/K', )
    #窗口.nextRow()
    if 1:  # 窗口内曲线4级
        θ = 右图4.plot(温度表, θ表, pen=None, name='θ表', symbol='o', symbolBrush='b', symbolSize=10)
