...
def 测量():
    初始 = time.time()
    电流 = 初始电流
    # Ls340_1.扫引控温(目标温度=270, 扫引速度K每min=0.1, 加热=3)  # 目標温度、変化速度、ヒーターのレンジを驱动.pyに入力
    # Ls340_2.扫引控温(目标温度=200,扫引速度K每min=1, 加热=5)
    K2182_1.触发切换('OFF')
    电压表, 相位表 = [], []
    if not os.path.exists(r'结果'):
        os.makedirs(r'结果')
    结果 = open(f'结果/{time.strftime("%Y年%m月%d日%H時%M分%S秒", time.localtime())}0.05HzI35mA.txt', mode='a', encoding='utf-8')
    结果.write('时间秒\t温度\tX表\tY表\n')
    if not os.path.exists(r'日志'):
        os.makedirs(r'日志')
    sys.stdout = 日志(f'日志/热导测量日志{time.strftime("%Y年%m月%d日%H時%M分%S秒", time.localtime())}0.05HzI35mA.log')
    while 1:
        时间1 = time.time()
        # if 时间1 - 開始 >= 72000:
        ...
        for i in range(19):
            try:
                time.sleep(0.5 * (i + 21) - (time.time() - 时间1))
            except:
                traceback.print_exc()
            else:
                电压表.append(K2182_1.触发读电压())
                相位表.append(i + 21)
        with 线程锁1:
            X表.append(sum(np.sin(2 * np.pi * np.array(相位表) / 40) * np.array(电压表)) / 38)
            Y表.append(-sum(np.cos(2 * np.pi * np.array(相位表) / 40) * np.array(电压表)) / 38)
        ...
        相位表.clear()
        try:
            time.sleep(0.5 * 40 - (time.time() - 时间1))
        except:
            traceback.print_exc()
