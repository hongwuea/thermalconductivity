...
f = open('结果/2022年06月22日13時14分52秒.txt', encoding='utf-8')
print(f.readline())
for 行 in f:
...
Cp49 = sliding_window_view(Cp表, 49)
Cpmean49 = np.mean(Cp49, axis=1)
Tem49 = sliding_window_view(温度表, 49)
Temmean49 = np.mean(Tem49, axis=1)
...
#        正交Y = 右图.plot(温度表, Y表, pen='b', name='正交Y', symbol='o', symbolBrush='b')
    ...
    右图3 = 窗口.addPlot(title="Cpmean曲线")
    右图3.setLabel(axis='left', text='信号/Volt')
    右图3.setLabel(axis='bottom', text='温度/K', )
    # 窗口.nextRow()
    if 1:  # 窗口内曲线4级
        R = 右图3.plot(Temmean49, Cpmean49, pen='b', name='R表', symbol='o', symbolBrush='b')
...
#print(R表.rolling(49,center=True).mean())
