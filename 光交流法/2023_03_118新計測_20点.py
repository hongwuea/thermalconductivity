...
# Ls350加熱出力番号 = 2
# Ls350_1.扫引控温(目标温度=80, 扫引速度K每min=0.1, 加热=5)      # 目標温度、変化速度、ヒーターのレンジを驱动.pyに入力
...
def 測定():
    ...
    while 1:
        ...
        with スレッドロック1:
            X表.append(sum(np.sin(2 * np.pi * np.array(位相表) / 標本数) * np.array(電圧表)) / 標本数)
            Y表.append(-sum(np.cos(2 * np.pi * np.array(位相表) / 標本数) * np.array(電圧表)) / 標本数)
